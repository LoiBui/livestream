...
const argv = minimst(process.argv.slice(2), {
    default: {
        ...
        ws_uri: 'ws://34.85.29.168:8888/kurento'
    }
});
...
io.on('connection', socket => {
    ...
    socket.on('disconnect', data => {
        // console.log(`Connection : %s disconnect`, socket.id);
    ...
    socket.on('message', message => {
        ...
        switch (message.id) {
            ...
            case 'onIceCandidate':
                ...
                break;
            case 'chat': 
                onChatMessage(socket, message, (error) => {
                    if (error) {
                        console.error(error);
                    }
                });
...
function join(socket, room, userName, callback) {
    ...
    room.pipeline.create('WebRtcEndpoint', (error, outgoingMedia) => {
        ...
        // else
        // outgoingMedia.setMaxVideoRecvBandwidth(900000);
        // outgoingMedia.setMinVideoRecvBandwidth(900000);
    ...
    if (incoming == null) {
        ...
        getRoom(userSession.roomName, (error, room) => {
            ...
            room.pipeline.create('WebRtcEndpoint', (error, incoming) => {
                ...
                // incoming.setMaxVideoRecvBandwidth(900000);
                // incoming.setMinVideoRecvBandwidth(900000);
...
function onChatMessage(socket, message, callback){
    var userSession = userRegister.getById(socket.id);

    if (!userSession) {
        callback(new Error("Can't find usersession"));
        return;
    }
